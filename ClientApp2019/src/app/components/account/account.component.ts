...
export class AccountComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.routeSub = this.activatedRoute.url.subscribe(() => {
            ...
            if (this.router.url === this.configService.routePaths.remindPassword) {
                this.showForm = true;
                this.viewType = AccountViewType.remind;

            } else if (this.router.url.split('?')[0] === this.configService.routePaths.resetPassword) {
        ...
        if (this.configService.applicationId === undefined && this.viewType !== AccountViewType.reset) {
    ...
    debounceCustomerName(customerName: string) {
        if (!this.companySubj.closed && customerName !== undefined) {
