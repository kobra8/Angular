    ...
    pricesAsync(i: number) {
        ...
        window.setTimeout(() => {
            ...
            if (this.configService.permissions.pricesVisibility
                && (this.productsList.products[i].netPrice === undefined || this.productsList.products[i].netPrice === '')
                && (this.productsList.products[i].grossPrice === undefined || this.productsList.products[i].grossPrice === '')) {
    ...
    addToCart(item: b2b.ProductListElement) {
        ...
        if (item.quantity && item.quantity > 0) {
            ...
            const productItem: b2b.AddToCartRequest = {
                ...
                calculateDiscount: this.configService.config.calculateDiscount && item.discountAllowed
            };
        ...
        if (products.length > 0) {
            ...
            const requestArray = products.map(item => {
                ...
                return <b2b.AddToCartRequest>{
                    id: cartId,
                    articleId: item.id, /* item.Id must be send as articleId  */
                    quantity: item.quantity,
