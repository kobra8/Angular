...
export class ProductsService extends ProductBase {
    ...
    */
    ...
    private pricesPromisesData: b2b.PromiseData<b2b.Prices>[];
    ...
    */
        ...
        const query: b2b.ItemsRequest = Object.assign(
            {
                ...
                warehouseId: this.filters.currentFilter.warehouse.id,
    ...
    loadProducts(groupId: number, cartId?: number): Promise<number> {
        ...
        return this.requestProducts(groupId, cartId).then(res => {
            ...
            if (productsRes.items && productsRes.items.set1 && productsRes.items.set1.length > 0) {
                ...
                productsRes.items.set1.forEach((product, i) => {
                    ...
                    this.pricesPromisesData[i] = pricesPromisesEl;
                    this.products[i] = this.calculateProductValues(product);
                    this.fillPricesAndUnitsData(i);
                });

            }

            this.pagination.changeParams(productsRes.paging);
            ...
            if (this.products.length > 0) {
                ...
                if (this.configService.applicationId === 0) {
                    this.changeWarehouseLocal = productsRes.items.set4[0].changeWarehouseEnabled;
                } else {
                    this.changeWarehouseLocal = productsRes.items.set3[0].changeWarehouseEnabled;
                }
    ...
    loadUnits(index: number): Promise<number> {
        ...
        return this.requestUnits(this.products[index].id).then((res: b2b.UnitResponse[]) => {
            ...
            this.fillUnits(index, res, this.products);
            return index;
    ...
    fillUnitMapElement(productIndex: number, unitId: number, unitData: b2b.UnitMapElement): b2b.UnitMapElement {
        ...
        return super.fillUnitMapElement(productIndex, unitId, unitData, this.products);

    }


    //fillBasicUnitPrices(productIndex: number) {

    //    const basicUnit = this.products[productIndex].units.get(0);

    //    if (basicUnit && (basicUnit.netPrice || basicUnit.grossPrice)) {
    //        this.products[productIndex].basicUnitNetPrice = basicUnit.netPrice;
    //        this.products[productIndex].basicUnitGrossPrice = basicUnit.grossPrice;
    //    } else {
    //        this.products[productIndex].basicUnitNetPrice = ConvertingUtils.calculateBasicUnitPrice(
    //            this.products[productIndex].netPrice, this.products[productIndex].denominator, this.products[productIndex].numerator
    //        );
    //        this.products[productIndex].basicUnitGrossPrice = ConvertingUtils.calculateBasicUnitPrice(
    //            this.products[productIndex].grossPrice, this.products[productIndex].denominator, this.products[productIndex].numerator
    //        );
    //    }
    ...
    */
    unitConverter(index: number): Promise<number> {
        ...
        return super.unitConverter(index, params, this.products);

    }

    private requestPricesAsync(index: number, quantity?: number): Promise<HttpResponse<b2b.ConvertedPricesResponse>> {
        ...
        return this.httpClient.get<b2b.PricesResponse>('/api/items/pricesasync/', { params: <any>query, observe: 'response' })
            .toPromise()
            .then(res => {


                const convertedPrices = this.convertProductPropertiesDifferences(res.body.set1[0]);

                const newSet = [Object.assign(res.body.set1[0], convertedPrices)];

                const newRes: any = res.clone(
                    {
                        body: Object.assign(res.body, { set1: newSet })
                    }
                );

                return newRes as HttpResponse<b2b.ConvertedPricesResponse>;
            }).catch(err => {
                return Promise.reject(err);
            });
    ...
    private requestPricesAsyncAltum(index: number): Promise<HttpResponse<b2b.ConvertedPricesResponse>> {
        ...
        return this.httpClient.get<b2b.PricesResponse>('/api/items/pricesasyncaltum/', { params: <any>query, observe: 'response' })
            .toPromise()
            .then(res => {
    ...
    pricesAsync(index: number): Promise<b2b.Prices> {

        let pricesPromise: Promise<HttpResponse<b2b.ConvertedPricesResponse>> = null;
        ...
        if (pricesPromise === null) {
            ...
            const fakeReponse = new HttpResponse<b2b.ConvertedPricesResponse>({
                status: 200,
                body: {
                    set1: [{
                        ...
                        unitLockChange: this.products[index].unitLockChange,
                        unitNetPrice: this.products[index].unitNetPrice,
                        unitGrossPrice: this.products[index].unitGrossPrice
                    }]
                }
            });
        ...
        return pricesPromise.then(res => {
            ...
            this.pricesPromisesData[index].promiseResolve({ prices: pricesResponse, index: index });

            return this.pricesPromisesData[index].promise;
            ...
            if (err.status === 204) {
                ...
                this.products[index].itemExistsInCurrentPriceList = false;

                this.pricesPromisesData[index].promiseReject({ prices: err, index: index });
    ...
    fillPricesAndUnitsData(productIndex: number) {

        Promise.all([this.unitsPromise, this.pricesPromisesData[productIndex].promise]).then(res => {
            ...
            for (const id in groupedUnits) {
                ...
                this.fillUnits(productIndex, groupedUnits[id], this.products);
            }
            ...
            if (!this.products[productIndex].units) {
                this.products[productIndex].units = new Map<number, b2b.UnitMapElement>();
            }
            ...
            if (this.configService.applicationId === 0) {
                this.products[productIndex].unitId = pricesResponse.defaultUnitNo;
            } else {
                this.products[productIndex].unitId = this.products[productIndex].basicUnitNo;
            }


            newUnitData = this.fillUnitMapElement(productIndex, this.products[productIndex].unitId, defaultUnitData);

            this.unitsLoaded = true;

            Object.assign(this.products[productIndex], pricesResponse, newUnitData);

            //this.fillBasicUnitPrices(productIndex);

            this.products[productIndex].pricesLoaded = true;
