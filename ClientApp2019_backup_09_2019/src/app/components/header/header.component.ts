...
export class HeaderComponent implements OnInit, OnDestroy {
    ...
    initLoggedUserHeader(): Promise<any> {
        ...
        this.carts.loadCartNumbers();

        const pendingMenuItemPromise = this.menuService.loadFullMenuItems().then(() => {
            this.pendingMenuItem = this.menuService.fullMenuItems.find(item => item.url === this.configService.routePaths.pending);
        });
        ...
        return Promise.all([cartPromise, pendingMenuItemPromise]);
    }
    ...
    search(formValid, formValue) {
        ...
        if (formValid) {
            ...
            if (trimmedValue.length > 0) {
                ...
                if (!location.href.includes('items') || location.href.toLowerCase().includes('details')) {

                    this.router.navigateByUrl(this.configService.routePaths.items).then(() => {
